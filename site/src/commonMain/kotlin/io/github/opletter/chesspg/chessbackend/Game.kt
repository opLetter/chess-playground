...
import io.github.opletter.chesspg.chessbackend.Square.Companion.asSquare
import io.github.opletter.chesspg.chessbackend.delegate.*
import io.github.opletter.chesspg.chessbackend.fen.FenParser
import io.github.opletter.chesspg.chessbackend.fen.FenParser.DEFAULT_POSITION
...
// my own extension
fun Game.getAllAvailableMoves(): Map<Square, List<Move>> {
    return state.board.findPiecesSquares { it.color == state.currentColor }.keys
        .associateWith { square -> availableMoves(square) }
}

fun Game.getAllAvailableMovesAsStrings(): Map<String, Array<String>> {
    return getAllAvailableMoves().map { (square, moves) ->
        square.toString() to moves.map {
            when (it) {
                is Move.CastleMove -> if (state.currentColor == Piece.Color.White) {
                    if (it.type == Move.CastleMove.CastleType.Short) "g1" else "c1"
                } else {
                    if (it.type == Move.CastleMove.CastleType.Short) "g8" else "c8"
                }

                is Move.GeneralMove -> Square(it.toFile, it.toRank).toString()
                is Move.PromotionMove -> Square(it.toFile, it.toRank).toString()
            }
        }.toTypedArray()
    }.toMap()
}

fun Game.move(from: String, to: String) {
    move(
        Move.create(
            piece = getPiece(from.asSquare()!!)!!,
            from = from.asSquare()!!,
            to = to.asSquare()!!
        )
    )
